...
export function highlightExpression(value, { stockSymbols = [] } = {}) {
    ...
    for (const match of matches) {
        ...
        const className = match.isStock ? 'token-stock'
            ...
            : /^(?:and|or|not|minus|plus|add|times|to|in|into|as|from|before|after|with|without|subtract|divide|multiplied|by|xor|mod|sum|total|product|quotient|difference|average|avg|mean|median|mode|of)$/i.test(token) ? 'token-connector'
            : '';
