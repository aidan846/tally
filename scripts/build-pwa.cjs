...
fs.writeFileSync(path.join(output, 'manifest.webmanifest'), JSON.stringify({ name: 'Tally', short_name: 'Tally', version: packageJson.version, start_url: './', display: 'standalone', background_color: '#111827', theme_color: '#111827', icons: [{ src: 'assets/icon.png', sizes: '512x512', type: 'image/png' }] }, null, 2));
fs.writeFileSync(path.join(output, 'sw.js'), `const CACHE = 'tally-v3'; self.addEventListener('install', event => event.waitUntil(caches.open(CACHE).then(cache => cache.addAll(['./', './index.html', './index.css', './renderer.js', './pwa-api.js', './mathjs/math.js'])))); self.addEventListener('fetch', event => event.respondWith(caches.match(event.request).then(hit => hit || fetch(event.request).then(response => { if (event.request.method === 'GET' && new URL(event.request.url).origin === location.origin) { const copy = response.clone(); caches.open(CACHE).then(cache => cache.put(event.request, copy)); } return response; }))));`);
